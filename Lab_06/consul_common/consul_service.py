...
class Consul:
    ...
    def __init__(self, port):
        self.port = port
        self.consul = consul.Consul(host='localhost', port=port)
    ...
    def hazelcast_map(self):
        ...
        _map = self.hazelcast.get_map(self.consul.kv.get('my-distributed-map')[1]['Value'].decode("utf-8")).blocking()
        return _map

    def hazelcast_queue(self):
        print("Connected to the hz clusters")
        _queue = self.hazelcast.get_queue(self.consul.kv.get('my-bounded-queue')[1]['Value'].decode("utf-8")).blocking()
        return _queue
